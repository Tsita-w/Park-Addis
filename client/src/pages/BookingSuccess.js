...
const BookingSuccess = () => {
  ...
  const [info, setInfo] = React.useState({ lot: "Parking Lot", slot: "--" });

  // 1. RE-ADD THIS LINE:
  const bookingRef = "PA-" + Math.floor(1000 + Math.random() * 9000);

  useEffect(() => {
    const bId = localStorage.getItem("currentBookingId");
    const sId = localStorage.getItem("currentSlotId");
    const lName = localStorage.getItem("currentLotName");
    const sLabel = localStorage.getItem("currentSlotLabel");

    if (lName) setInfo({ lot: lName, slot: sLabel });

    const finalize = async () => {
      if (bId && sId) {
        await fetch("http://localhost:5000/api/bookings/finalize", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ bookingId: bId, slotId: sId }),
        });
      }
    };

    finalize();
    toast.success(`Verified! Welcome to ${lName || "Park Addis"}.`);
  }, []);
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6">
      <div className="bg-white p-8 rounded-3xl shadow-xl max-w-md w-full text-center border border-gray-100">
        ...
        <div className="p-3 bg-yellow-50 rounded-xl border border-yellow-200 mb-6 text-center">
          <p className="text-[10px] text-yellow-800 font-bold uppercase tracking-widest mb-1">
            Reference
          </p>
